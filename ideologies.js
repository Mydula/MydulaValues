ideologies = [
    {
        "name": "Анархо-коммунизм",
        "stats": {
            "econ": 100,
            "dipl": 50,
            "govt": 100,
            "scty": 90
        }
    },
    {
        "name": "Либертарный марксизм",
        "stats": {
            "econ": 100,
            "dipl": 70,
            "govt": 80,
            "scty": 80
        }
    },
    {
        "name": "Троцкизм",
        "stats": {
            "econ": 100,
            "dipl": 100,
            "govt": 50,
            "scty": 80
        }
    },
    {
        "name": "Марксизм",
        "stats": {
            "econ": 100,
            "dipl": 70,
            "govt": 40,
            "scty": 80
        }
    },
    {
        "name": "Неомарксизм",
        "stats": {
            "econ": 100,
            "dipl": 90,
            "govt": 55,
            "scty": 90
        }
    },
    {
        "name": "Люксембургианство",
        "stats": {
            "econ": 100,
            "dipl": 70,
            "govt": 70,
            "scty": 70
        }
    },    
    {
        "name": "Де Леонизм",
        "stats": {
            "econ": 100,
            "dipl": 50,
            "govt": 60,
            "scty": 80
        }
    },
    {
        "name": "Ленинизм",
        "stats": {
            "econ": 90,
            "dipl": 60,
            "govt": 35,
            "scty": 80
        }
    },
    {
        "name": "Марксизм-ленинизм",
        "stats": {
            "econ": 90,
            "dipl": 50,
            "govt": 20,
            "scty": 60
        }
    },
    {
        "name": "Национал-коммунизм",
        "stats": {
            "econ": 100,
            "dipl": 20,
            "govt": 30,
            "scty": 55
        }
    },    
    {
        "name": "Религиозный коммунизм",
        "stats": {
            "econ": 100,
            "dipl": 50,
            "govt": 30,
            "scty": 30
        }
    },
    {
        "name": "Утопический социализм",
        "stats": {
            "econ": 80,
            "dipl": 70,
            "govt": 70,
            "scty": 90
        }
    },    
    {
        "name": "Государственный социализм",
        "stats": {
            "econ": 80,
            "dipl": 50,
            "govt": 20,
            "scty": 40
        }
    },
    {
        "name": "Консервативный социализм",
        "stats": {
            "econ": 80,
            "dipl": 40,
            "govt": 30,
            "scty": 20
        }
    },
    {
        "name": "Религиозный социализм",
        "stats": {
            "econ": 80,
            "dipl": 50,
            "govt": 60,
            "scty": 20
        }
    },
    {
        "name": "Демократический социализм",
        "stats": {
            "econ": 80,
            "dipl": 60,
            "govt": 50,
            "scty": 80
        }
    },
    {
        "name": "Революционный социализм",
        "stats": {
            "econ": 90,
            "dipl": 40,
            "govt": 50,
            "scty": 70
        }
    },
    {
        "name": "Либеральный социализм",
        "stats": {
            "econ": 80,
            "dipl": 60,
            "govt": 60,
            "scty": 80
        }
    },    
    {
        "name": "Либертарный социализм",
        "stats": {
            "econ": 80,
            "dipl": 60,
            "govt": 80,
            "scty": 80
        }
    },
    {
        "name": "Национал-большевизм",
        "stats": {
            "econ": 80,
            "dipl": 10,
            "govt": 5,
            "scty": 20
        }
    },    
    {
        "name": "Анархо-синдикализм",
        "stats": {
            "econ": 70,
            "dipl": 50,
            "govt": 100,
            "scty": 80
        }
    },
    {
        "name": "Национал-синдикализм",
        "stats": {
            "econ": 70,
            "dipl": 10,
            "govt": 10,
            "scty": 20
        }
    },    
    {
        "name": "Левый популизм",
        "stats": {
            "econ": 70,
            "dipl": 60,
            "govt": 50,
            "scty": 70
        }
    },
    {
        "name": "Левый национализм",
        "stats": {
            "econ": 70,
            "dipl": 20,
            "govt": 40,
            "scty": 40
        }
    },
    {
        "name": "Патерналистский консерватизм",
        "stats": {
            "econ": 60,
            "dipl": 30,
            "govt": 30,
            "scty": 20
        }
    },
    {
        "name": "Дистрибутизм",
        "stats": {
            "econ": 55,
            "dipl": 50,
            "govt": 40,
            "scty": 20
        }
    },
    {
        "name": "Социал-либерализм",
        "stats": {
            "econ": 60,
            "dipl": 60,
            "govt": 60,
            "scty": 80
        }
    },
    {
        "name": "Религиозная демократия",
        "stats": {
            "econ": 60,
            "dipl": 50,
            "govt": 50,
            "scty": 30
        }
    },
    {
        "name": "Социал-демократия",
        "stats": {
            "econ": 60,
            "dipl": 70,
            "govt": 55,
            "scty": 80
        }
    },
    {
        "name": "Социальный авторитаризм",
        "stats": {
            "econ": 60,
            "dipl": 50,
            "govt": 20,
            "scty": 50
        }
    },
    {
        "name": "Социальный национализм",
        "stats": {
            "econ": 60,
            "dipl": 10,
            "govt": 30,
            "scty": 30
        }
    },    
    {
        "name": "Прогрессивизм",
        "stats": {
            "econ": 60,
            "dipl": 80,
            "govt": 60,
            "scty": 100
        }
    },
    {
        "name": "Социальный анархизм",
        "stats": {
            "econ": 60,
            "dipl": 70,
            "govt": 100,
            "scty": 70
        }
    },    
    {
        "name": "Мютюэлизм",
        "stats": {
            "econ": 55,
            "dipl": 50,
            "govt": 100,
            "scty": 70
        }
    },
    {
        "name": "Тоталитаризм",
        "stats": {
            "econ": 50,
            "dipl": 50,
            "govt": 0,
            "scty": 50
        }
    },
    {
        "name": "Национал-анархизм",
        "stats": {
            "econ": 60,
            "dipl": 20,
            "govt": 90,
            "scty": 30
        }
    },
    {
        "name": "Анархо-нигилизм",
        "stats": {
            "econ": 65,
            "dipl": 100,
            "govt": 100,
            "scty": 100
        }
    },
    {
        "name": "Центризм",
        "desc": "Центризм - это идеология, которая заключается в балансировании идей всех сторон: левых, правых, либертарных и авторитарных, и находится в центре политического компаса.",
        "stats": {
            "econ": 50,
            "dipl": 50,
            "govt": 50,
            "scty": 50
        }
    },
    {
        "name": "Либеральная демократия",
        "stats": {
            "econ": 50,
            "dipl": 60,
            "govt": 60,
            "scty": 60
        }
    },
    {
        "name": "Национал-либерализм",
        "stats": {
            "econ": 30,
            "dipl": 20,
            "govt": 60,
            "scty": 60
        }
    },   
    {
        "name": "Религиозный анархизм",
        "stats": {
            "econ": 50,
            "dipl": 50,
            "govt": 100,
            "scty": 20
        }
    },
    {
        "name": "Правый популизм",
        "stats": {
            "econ": 30,
            "dipl": 20,
            "govt": 40,
            "scty": 25
        }
    },
    {
        "name": "Национал-демократия",
        "stats": {
            "econ": 40,
            "dipl": 15,
            "govt": 50,
            "scty": 20
        }
    },    
    {
        "name": "Либеральный консерватизм",
        "stats": {
            "econ": 30,
            "dipl": 50,
            "govt": 55,
            "scty": 40
        }
    },
    {
        "name": "Политическая реакция",
        "stats": {
            "econ": 30,
            "dipl": 40,
            "govt": 30,
            "scty": 0
        }
    },
    {
        "name": "Неореакция (тёмное просвещение)",
        "stats": {
            "econ": 15,
            "dipl": 20,
            "govt": 20,
            "scty": 5
        }
    },   
    {
        "name": "Социальное либертарианство",
        "stats": {
            "econ": 60,
            "dipl": 70,
            "govt": 80,
            "scty": 70
        }
    },
    {
        "name": "Либертарианство",
        "stats": {
            "econ": 30,
            "dipl": 60,
            "govt": 80,
            "scty": 60
        }
    },
    {
        "name": "Эгоизм",
        "stats": {
            "econ": 40,
            "dipl": 50,
            "govt": 100,
            "scty": 50
        }
    },
    {
        "name": "Национал-социализм",
        "stats": {
            "econ": 45,
            "dipl": 0,
            "govt": 0,
            "scty": 10
        }
    },
    {
        "name": "Штрассеризм",
        "stats": {
            "econ": 65,
            "dipl": 20,
            "govt": 15,
            "scty": 20
        }
    },    
    {
        "name": "Автократия",
        "stats": {
            "econ": 50,
            "dipl": 20,
            "govt": 20,
            "scty": 50
        }
    },
    {
        "name": "Фашизм",
        "stats": {
            "econ": 60,
            "dipl": 10,
            "govt": 5,
            "scty": 20
        }
    },
    {
        "name": "Клерикальный фашизм",
        "stats": {
            "econ": 55,
            "dipl": 10,
            "govt": 5,
            "scty": 10
        }
    },
    {
        "name": "Эзотерический фашизм",
        "stats": {
            "econ": 50,
            "dipl": 10,
            "govt": 5,
            "scty": 0
        }
    },
    {
        "name": "Футуризм",
        "stats": {
            "econ": 65,
            "dipl": 10,
            "govt": 15,
            "scty": 95
        }
    },
    {
        "name": "Национал-капитализм",
        "stats": {
            "econ": 20,
            "dipl": 20,
            "govt": 5,
            "scty": 40
        }
    },
    {
        "name": "Консерватизм",
        "stats": {
            "econ": 30,
            "dipl": 40,
            "govt": 40,
            "scty": 20
        }
    },
    {
        "name": "Авторитарный консерватизм",
        "stats": {
            "econ": 30,
            "dipl": 30,
            "govt": 20,
            "scty": 20
        }
    },
    {
        "name": "Национал-консерватизм",
        "stats": {
            "econ": 30,
            "dipl": 15,
            "govt": 25,
            "scty": 20
        }
    },    
    {
        "name": "Неолиберализм",
        "stats": {
            "econ": 30,
            "dipl": 40,
            "govt": 50,
            "scty": 60
        }
    },
    {
        "name": "Третий путь (социал-неолиберализм)",
        "stats": {
            "econ": 40,
            "dipl": 50,
            "govt": 50,
            "scty": 80
        }
    },    
    {
        "name": "Классический либерализм",
        "stats": {
            "econ": 30,
            "dipl": 60,
            "govt": 60,
            "scty": 80
        }
    },
    {
        "name": "Консервативный либерализм",
        "stats": {
            "econ": 25,
            "dipl": 40,
            "govt": 60,
            "scty": 30
        }
    },
    {
        "name": "Реакционный либерализм",
        "stats": {
            "econ": 20,
            "dipl": 20,
            "govt": 60,
            "scty": 10
        }
    },
    {
        "name": "Авторитарный капитализм",
        "stats": {
            "econ": 20,
            "dipl": 40,
            "govt": 20,
            "scty": 50
        }
    },
    {
        "name": "Государственный капитализм",
        "stats": {
            "econ": 50,
            "dipl": 50,
            "govt": 20,
            "scty": 50
        }
    },
    {
        "name": "Неоконсерватизм",
        "stats": {
            "econ": 25,
            "dipl": 30,
            "govt": 40,
            "scty": 35
        }
    },
    {
        "name": "Палеоконсерватизм",
        "stats": {
            "econ": 25,
            "dipl": 20,
            "govt": 30,
            "scty": 20
        }
    },
    {
        "name": "Правое либертарианство",
        "stats": {
            "econ": 20,
            "dipl": 50,
            "govt": 80,
            "scty": 60
        }
    },
    {
        "name": "Либертарианский консерватизм",
        "stats": {
            "econ": 30,
            "dipl": 50,
            "govt": 80,
            "scty": 30
        }
    },
    {
        "name": "Сострадательное либертарианство",
        "stats": {
            "econ": 30,
            "dipl": 60,
            "govt": 80,
            "scty": 80
        }
    },
    {
        "name": "Палеолибертарианство",
        "stats": {
            "econ": 20,
            "dipl": 30,
            "govt": 75,
            "scty": 30
        }
    },
    {
        "name": "Национал-либертарианство",
        "stats": {
            "econ": 30,
            "dipl": 20,
            "govt": 80,
            "scty": 50
        }
    },
    {
        "name": "Реакционное либертарианство",
        "stats": {
            "econ": 10,
            "dipl": 30,
            "govt": 75,
            "scty": 10
        }
    },
    {
        "name": "Агоризм",
        "stats": {
            "econ": 30,
            "dipl": 60,
            "govt": 90,
            "scty": 70
        }
    },
    {
        "name": "Рыночный анархизм",
        "stats": {
            "econ": 20,
            "dipl": 50,
            "govt": 100,
            "scty": 50
        }
    },
    {
        "name": "Объективизм",
        "stats": {
            "econ": 10,
            "dipl": 50,
            "govt": 90,
            "scty": 40
        }
    },
    {
        "name": "Минархизм",
        "stats": {
            "econ": 10,
            "dipl": 50,
            "govt": 75,
            "scty": 50
        }
    },    
    {
        "name": "Традиционализм",
        "stats": {
            "econ": 30,
            "dipl": 30,
            "govt": 10,
            "scty": 10
        }
    },
    {
        "name": "Фискальный консерватизм",
        "stats": {
            "econ": 10,
            "dipl": 50,
            "govt": 50,
            "scty": 30
        }
    },    
    {
        "name": "Корпоратократия",
        "stats": {
            "econ": 0,
            "dipl": 50,
            "govt": 40,
            "scty": 50
        }
    },
    {
        "name": "Анархо-капитализм",
        "stats": {
            "econ": 0,
            "dipl": 50,
            "govt": 100,
            "scty": 50
        }
    }
];
